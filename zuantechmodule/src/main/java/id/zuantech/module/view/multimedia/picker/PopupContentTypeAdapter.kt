...
class PopupContentTypeAdapter(private val onClick:(PickerType) -> Unit)
    : RecyclerView.Adapter<PopupContentTypeAdapter.ViewHolder>() {

    var items : List<PickerType> = emptyList()
    set(value) {
        field = value
    ...
    fun tintColor(tintColor: Int?): PopupContentTypeAdapter {
    ...
    inner class ViewHolder(containerView: View) : RecyclerView.ViewHolder(containerView){
        ...
        fun bind(data: PickerType, position: Int){
            with(mBinding){
