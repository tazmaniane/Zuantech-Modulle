...
import id.zuantech.module.databinding.HeaderWhiteViewBinding

class HeaderWhiteView : FrameLayout {

    companion object {
        val TAG = HeaderWhiteView::class.java.simpleName
    }

    private var mBinding = HeaderWhiteViewBinding.inflate(LayoutInflater.from(context), this, true)
    ...
    fun setHeaderUserViewListener(headerUserViewListener: HeaderUserViewListener): HeaderWhiteView {
        this.headerUserViewListener = headerUserViewListener
        return this
    }

    fun setTitle(title: String?): HeaderWhiteView {
    ...
    fun setTitle(title: String?, isTransparent: Boolean): HeaderWhiteView {
        mBinding.lineHeaderTitle.visibility = GONE
        title?.let {
            mBinding.tvHeaderTitle.text = it
            mBinding.lineHeaderTitle.visibility = VISIBLE
...
//    }
    ...
    fun isShowBackButton(isShowBackButton: Boolean): HeaderWhiteView {
        mBinding.btnBack.visibility = if(isShowBackButton) VISIBLE else GONE
        return this
    }

    fun isShowBackButton(isShowBackButton: Boolean, onClick: () -> Unit): HeaderWhiteView {
        mBinding.btnBack.visibility = if(isShowBackButton) VISIBLE else GONE
        if(isShowBackButton){
...
//    }
    ...
    fun setAction1Content(title: String?, icon: Int?, onClick: () -> Unit): HeaderWhiteView {
    ...
    fun setActionDeleteContent(onClick: () -> Unit): HeaderWhiteView {
